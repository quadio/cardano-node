...
import           Cardano.CLI.Shelley.Key (InputDecodeError, OutputDirection,
                     VerificationKeyOrFile, readVerificationKeyOrFile, serialiseInputToBech32AndWrite)
...
runPoolCmd (PoolGetId sPvkey outputDirection) =
  runPoolId sPvkey outputDirection
...
runPoolId
  ...
  -> OutputDirection
  -> ExceptT ShelleyPoolCmdError IO ()
runPoolId verKeyOrFile outputDirection = do
  stakePoolVerKey <- firstExceptT ShelleyPoolCmdReadKeyFileError
    . newExceptT
